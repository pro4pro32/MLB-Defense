...
# Below this many opportunities in a bucket, the avg-CP estimate is too noisy
# to be meaningful (a single play can swing it by 20+ points) — show "—" instead.
MIN_BUCKET_SAMPLE: int = 5

# ── Park factors (optional, user-supplied) ────────────────────────────────────
# We deliberately do NOT hardcode specific park-factor numbers here. Real park
# factors are researched figures (e.g. from FanGraphs / Baseball Reference)
# that should be sourced and verified by whoever maintains this dashboard, not
# invented by this code. Every team defaults to a neutral factor of 1.00 —
# i.e. no adjustment — until real numbers are supplied via an optional CSV:
#
#   data/raw/park_factors.csv   with columns:  team,factor
#
# `team` should match the `team` column in the sprint-speed data (whatever
# format Baseball Savant uses — abbreviation or full name). `factor` > 1.00
# implies a park that suppresses defensive difficulty relative to average
# (so OAA is adjusted up); < 1.00 implies the opposite. Adjust the direction
# in `_add_park_adjustment` below if your source defines it the other way.
PARK_FACTORS_PATH = RAW_DIR / "park_factors.csv"
...
def _load_ss_season(year: int) -> Optional[pd.DataFrame]:
    ...
    if df is not None:
        ...
        df = _fetch_ss_online(year)
    if df is None:
        return None
    return _augment_with_jump_metrics(df, year)
...
def fetch_cp_plays_scaffold(year: int) -> Optional[pd.DataFrame]:
    """
    EXPERIMENTAL / SCAFFOLDING ONLY — see caveat below.

    Fetches per-play Statcast batted-ball context for `year` via
    pybaseball.statcast() and saves it to
    data/raw/catch_prob_plays_{year}.parquet.

    IMPORTANT: Baseball Savant's public per-play feed does not include a
    ready-made catch-probability value per batted ball — that figure is
    Savant's internal model output, published only in aggregate on the
    catch-probability leaderboard (which is exactly what
    catch_prob_{year}.parquet already contains, and what _estimate_avg_cp
    approximates from). This function stores the raw batted-ball context
    (launch speed/angle, hit distance, hang time proxies) that a *custom*
    catch-probability model would need as input — it does not itself
    produce a validated per-play CP value. The dashboard's avg_cp columns
    keep using _estimate_avg_cp() regardless of whether this has been run.

    Treat this as a starting point for building your own model, not a
    drop-in replacement for the current estimate.
    """
    try:
        from pybaseball import statcast, cache as pb
        pb.enable()
        raw = statcast(start_dt=f"{year}-03-01", end_dt=f"{year}-11-30")
        if raw is None or raw.empty:
            return None
        keep_cols = [c for c in [
            "game_date", "player_name", "fielder_2", "bb_type",
            "hit_distance_sc", "launch_speed", "launch_angle", "hc_x", "hc_y",
        ] if c in raw.columns]
        if not keep_cols:
            return None
        plays = raw[keep_cols].copy()
        plays["season"] = year
        RAW_DIR.mkdir(parents=True, exist_ok=True)
        out = RAW_DIR / f"catch_prob_plays_{year}.parquet"
        plays.to_parquet(out, index=False)
        log.info("Saved catch_prob_plays_%d (%d rows) — scaffolding only, see docstring.",
                 year, len(plays))
        return plays
    except Exception as exc:
        log.warning("Per-play CP scaffold fetch failed for %d (non-fatal): %s", year, exc)
        return None
...
# Candidate raw column names for jump/reaction metrics, since the exact
# schema depends on which Statcast leaderboard export is used.
_JUMP_COL_CANDIDATES: dict[str, list[str]] = {
    "reaction_distance": ["reaction_distance", "r_reaction", "reaction"],
    "burst_distance":    ["burst_distance", "r_burst", "burst"],
}


def _augment_with_jump_metrics(ss_df: pd.DataFrame, year: int) -> pd.DataFrame:
    """
    Left-join optional outfielder 'jump' metrics (reaction distance, burst
    distance) onto the sprint-speed frame for this season, if available
    locally or fetchable online. Entirely optional: if no jump data can be
    found, ss_df is returned unchanged and downstream code simply shows
    "—" for these fields (see _process_sprint / render_player_detail_tab).
    """
    jump_path = RAW_DIR / f"outfielder_jump_{year}.parquet"
    jump_df = _read_parquet(jump_path)
    if jump_df is None:
        jump_df = _fetch_jump_online(year)
    if jump_df is None or jump_df.empty or "player_id" not in jump_df.columns:
        return ss_df

    rename = {}
    for out_col, candidates in _JUMP_COL_CANDIDATES.items():
        src = _find_col(jump_df, candidates)
        if src:
            rename[src] = out_col
    if not rename:
        return ss_df

    slim = (jump_df[["player_id"] + list(rename.keys())]
            .rename(columns=rename)
            .drop_duplicates(subset=["player_id"]))
    return ss_df.merge(slim, on="player_id", how="left")


def _fetch_jump_online(year: int) -> Optional[pd.DataFrame]:
    """
    Attempt to fetch Statcast's 'outfielder jump' leaderboard (reaction
    distance / burst distance). Not every pybaseball version wraps this
    leaderboard, so we probe for the function and degrade gracefully —
    a missing jump leaderboard should never break the rest of the app.
    """
    try:
        import pybaseball as pb
        fetch_fn = getattr(pb, "statcast_outfielder_jump", None)
        if fetch_fn is None:
            log.info("pybaseball has no outfielder-jump leaderboard in this version — skipping.")
            return None
        df = fetch_fn(year=year, min_att=1)
        if df is None or df.empty:
            return None
        RAW_DIR.mkdir(parents=True, exist_ok=True)
        df.to_parquet(RAW_DIR / f"outfielder_jump_{year}.parquet", index=False)
        log.info("Fetched and saved outfielder_jump_%d (%d rows).", year, len(df))
        return df
    except Exception as exc:
        log.warning("Jump-metric fetch failed for %d (non-fatal): %s", year, exc)
        return None
...
def build_dashboard_df(cp_raw: pd.DataFrame, ss_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    Top-level columns also include:
      park_factor   — 1.00 (neutral) unless data/raw/park_factors.csv supplies
                      real per-team figures (see _add_park_adjustment)
      park_adj_oaa  — oaa × park_factor; equals oaa when no real factors loaded
    """
    if cp_raw.empty:
        return pd.DataFrame()
...
def _load_park_factors() -> dict[str, float]:
    """
    Load optional real park factors from data/raw/park_factors.csv
    (columns: team,factor). Returns {} if the file doesn't exist, in which
    case every team is treated as neutral (factor 1.00) — see the comment
    on PARK_FACTORS_PATH for why we don't hardcode figures here.
    """
    if not PARK_FACTORS_PATH.exists():
        return {}
    try:
        pf = pd.read_csv(PARK_FACTORS_PATH)
        if "team" not in pf.columns or "factor" not in pf.columns:
            log.warning("park_factors.csv found but missing 'team'/'factor' columns — ignoring.")
            return {}
        return dict(zip(pf["team"], pf["factor"].apply(safe_float)))
    except Exception as exc:
        log.warning("Could not load park_factors.csv (%s) — using neutral factors.", exc)
        return {}


def _add_park_adjustment(df: pd.DataFrame) -> pd.DataFrame:
    """
    Add park_factor / park_adj_oaa columns. Neutral (1.00 / oaa unchanged)
    unless data/raw/park_factors.csv is present with real figures.
    """
    factors = _load_park_factors()
    if not factors or "team" not in df.columns:
        df["park_factor"]  = 1.0
        df["park_adj_oaa"] = df.get("oaa", 0)
        return df
    df["park_factor"]  = df["team"].map(factors).fillna(1.0)
    df["park_adj_oaa"] = (df["oaa"] * df["park_factor"]).round(2)
    return df
...
def _process_catch_prob(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        for b in BUCKETS:
            ...
            avg_cp    = (_estimate_avg_cp(b, catch_pct)
                         if (att >= MIN_BUCKET_SAMPLE and not np.isnan(catch_pct))
                         else np.nan)
...
def _process_sprint(raw: pd.DataFrame) -> pd.DataFrame:
    if raw.empty:
        return pd.DataFrame(columns=["player_id","season","sprint_speed",
                                     ...
                                     "reaction_distance","burst_distance"])
    df  = raw.copy()
    nc  = _find_col(df, ["last_name, first_name", "player_name", "name"])
    if nc:
    ...
    keep = {"player_id":"player_id","sprint_speed":"sprint_speed",
            ...
            # Optional outfielder-jump metrics — present only if
            # _augment_with_jump_metrics found a jump leaderboard for this
            # season; otherwise simply absent and downstream shows "—".
            "reaction_distance":"reaction_distance","burst_distance":"burst_distance"}
    ...
    for col, fn in [("sprint_speed", safe_float), ("age", safe_float),
                    ...
                    ("reaction_distance", safe_float), ("burst_distance", safe_float)]:
...
def _merge(cp: pd.DataFrame, ss: pd.DataFrame) -> pd.DataFrame:
    passthrough = ["sprint_speed","position","team","age","bolts",
                   "reaction_distance","burst_distance"]
    if ss.empty:
        for c in passthrough:
            cp[c] = np.nan
        return cp
    keep = ["player_id","season"] + passthrough
...
def _make_oaa_waterfall_fig(row: pd.Series, label: str, season) -> go.Figure:
    """Build the per-bracket OAA waterfall bar chart for one player-season row."""
    labels, vals, colors = [], [], []
    for b in BUCKETS:
        v = float(row.get(f"b{b.id}_oaa") or 0)
        labels.append(b.label)
        vals.append(v)
        colors.append("#0F6E56" if v >= 0 else "#993C1D")
    labels.append("Total OAA")
    vals.append(float(row.get("oaa", 0) or 0))
    colors.append("#1D3557")
    fig = go.Figure(go.Bar(
        x=labels, y=vals, marker_color=colors,
        text=[f"{v:+.2f}" if i < len(BUCKETS) else f"{v:+.0f}" for i, v in enumerate(vals)],
        textposition="outside", textfont=dict(size=11),
    ))
    fig.add_hline(y=0, line_color="rgba(0,0,0,0.2)", line_width=1)
    fig.update_layout(height=310, plot_bgcolor="white", paper_bgcolor="white",
                       font_family="system-ui", margin=dict(t=30, b=10, l=10, r=10),
                       yaxis=dict(showgrid=True, gridcolor="#eee", title="OAA"),
                       xaxis=dict(showgrid=False),
                       title=dict(text=f"{label} — {season}", font=dict(size=13)))
    return fig


def _player_kpi_summary(row: pd.Series) -> dict:
    """Compute the same headline metrics used in the header KPI cards, as raw values."""
    ...
    return {
        "Total OAA":         float(row.get("oaa", 0) or 0),
        "Opportunities":     total_att,
        "Sprint Speed":      float(row["sprint_speed"]) if pd.notna(row.get("sprint_speed")) else np.nan,
        "Overall catch%":    (total_suc / total_att * 100) if total_att > 0 else np.nan,
        "Expected catch%":   (total_exp / total_att * 100) if total_att > 0 else np.nan,
    }


def _render_head_to_head(
    row_a: pd.Series, name_a: str, season_a,
    row_b: pd.Series, name_b: str, season_b,
) -> None:
    """Render a compact head-to-head comparison: KPI table, bracket catch% bars, OAA waterfalls."""
    st.markdown("<div class='section-head'>🆚 Head-to-head — KPI comparison</div>",
                unsafe_allow_html=True)

    a_kpi = _player_kpi_summary(row_a)
    b_kpi = _player_kpi_summary(row_b)
    col_a_label = f"{name_a} ({season_a})"
    col_b_label = f"{name_b} ({season_b})"

    fmt_by_metric = {
        "Total OAA":       fmt_oaa,
        "Opportunities":   fmt_int,
        "Sprint Speed":    fmt_speed,
        "Overall catch%":  fmt_pct,
        "Expected catch%": fmt_pct,
    }
    comp_rows = []
    for metric, a_val in a_kpi.items():
        b_val = b_kpi[metric]
        fmt_fn = fmt_by_metric[metric]
        diff = (a_val - b_val) if (pd.notna(a_val) and pd.notna(b_val)) else np.nan
        diff_disp = f"{'+' if diff >= 0 else ''}{diff:.2f}" if not np.isnan(diff) else "—"
        comp_rows.append({
            "Metric":     metric,
            col_a_label:  fmt_fn(a_val),
            col_b_label:  fmt_fn(b_val),
            "Δ (A − B)":  diff_disp,
        })
    st.dataframe(pd.DataFrame(comp_rows), hide_index=True, use_container_width=True, height=230)

    st.markdown("<div class='section-head'>Catch% by bracket — both players</div>",
                unsafe_allow_html=True)
    blabels = [b.label for b in BUCKETS]
    a_pct = [float(row_a.get(f"b{b.id}_catch_pct") or 0) for b in BUCKETS]
    b_pct = [float(row_b.get(f"b{b.id}_catch_pct") or 0) for b in BUCKETS]
    fig_cmp = go.Figure()
    fig_cmp.add_trace(go.Bar(name=col_a_label, x=blabels, y=a_pct, marker_color="#1D3557",
                              text=[f"{v:.1f}%" for v in a_pct],
                              textposition="outside", textfont=dict(size=10)))
    fig_cmp.add_trace(go.Bar(name=col_b_label, x=blabels, y=b_pct, marker_color="#D85A30",
                              text=[f"{v:.1f}%" for v in b_pct],
                              textposition="outside", textfont=dict(size=10)))
    fig_cmp.add_trace(go.Scatter(x=blabels, y=[b.exp_pct for b in BUCKETS],
                                  mode="markers+lines", name="Expected",
                                  marker=dict(symbol="diamond", size=8, color="#5A9E6F"),
                                  line=dict(color="#5A9E6F", dash="dot", width=1.5)))
    fig_cmp.update_layout(barmode="group", height=380, plot_bgcolor="white", paper_bgcolor="white",
    ...
    st.markdown("<div class='section-head'>OAA waterfall — side by side</div>",
                unsafe_allow_html=True)
    wf_col_a, wf_col_b = st.columns(2)
    with wf_col_a:
        st.plotly_chart(_make_oaa_waterfall_fig(row_a, name_a, season_a), use_container_width=True)
    with wf_col_b:
        st.plotly_chart(_make_oaa_waterfall_fig(row_b, name_b, season_b), use_container_width=True)

    st.caption(
        "**Δ (A − B)** = Player A minus Player B; positive means Player A is ahead on that metric. "
        f"“A” = {col_a_label}, “B” = {col_b_label}."
    )
...
def render_player_detail_tab(df_all: pd.DataFrame) -> None:
    ...
    # Optional outfielder-jump metrics (reaction / burst distance) — only
    # rendered when the underlying data is available for this player-season
    # (see _augment_with_jump_metrics). Kept as a caption rather than a KPI
    # tile so its absence never disturbs the header layout.
    jump_bits = []
    if pd.notna(row.get("reaction_distance")):
        jump_bits.append(f"Reaction distance: {float(row['reaction_distance']):.1f} ft")
    if pd.notna(row.get("burst_distance")):
        jump_bits.append(f"Burst distance: {float(row['burst_distance']):.1f} ft")
    if jump_bits:
        st.caption("🏃 " + " · ".join(jump_bits))

    st.divider()

    # ── Optional: compare with a second player ────────────────────────────────
    compare_on = st.checkbox("🆚 Compare with a second player", key="pd_compare_on")
    if compare_on:
        comp_pool = [p for p in all_players if p != chosen_player] or all_players
        cc1, cc2 = st.columns([3, 1])
        with cc1:
            comp_player = st.selectbox("Compare against", comp_pool, index=0, key="pd_compare_player")
        with cc2:
            comp_seasons = sorted(
                df_all.loc[df_all["player_name"] == comp_player, "season"]
                .dropna().unique().astype(int).tolist()
            )
            comp_season = st.selectbox("Season", comp_seasons,
                                       index=len(comp_seasons) - 1, key="pd_compare_season")

        comp_mask = (df_all["player_name"] == comp_player) & (df_all["season"] == comp_season)
        comp_subset = df_all[comp_mask]
        if comp_subset.empty:
            st.warning(f"No data for {comp_player} in {comp_season}.")
        else:
            _render_head_to_head(row, chosen_player, chosen_season,
                                 comp_subset.iloc[0], comp_player, comp_season)
        st.divider()
    ...
    with col_wf:
        ...
        st.plotly_chart(_make_oaa_waterfall_fig(row, chosen_player, chosen_season),
...
def _df_hash(df: pd.DataFrame) -> str:
    """
    Real content hash for a DataFrame, used as a cache key.

    Replaces the old `len(df)` proxy, which silently failed to invalidate
    the cache whenever the row count stayed the same but values changed
    (e.g. a corrected stat, a re-fetched season with the same opportunity
    count). hash_pandas_object hashes every row+column; summing then
    hex-digesting keeps the key small and stable.
    """
    if df is None or df.empty:
        return "empty"
    try:
        ...
        # Fallback for any unhashable dtype edge case — still better than len()
        return hashlib.md5(pd.util.hash_pandas_object(
            df.astype(str), index=True
        ).values).hexdigest()
...
def _cached_dashboard(cp_hash: str, ss_hash: str, _cp, _ss):
...
with st.sidebar:
    ...
    st.divider()
    with st.expander("🧪 Experimental: per-play CP scaffolding"):
        st.caption(
            "Fetches raw per-play batted-ball context via pybaseball.statcast() and "
            "saves it to data/raw/catch_prob_plays_{year}.parquet, as a starting point "
            "for building a custom catch-probability model. **This does not change any "
            "numbers shown in the dashboard today** — Avg CP% (est.) still uses the "
            "existing bucket-midpoint estimate. Can be slow (full-season pull)."
        )
        scaffold_year = st.selectbox("Season to fetch", SEASONS, index=len(SEASONS) - 1,
                                     key="scaffold_yr")
        if st.button("Fetch per-play data for this season", use_container_width=True):
            with st.spinner(f"Fetching per-play data for {scaffold_year}… this can take a while."):
                scaffold_result = fetch_cp_plays_scaffold(scaffold_year)
            if scaffold_result is not None:
                st.success(
                    f"Saved {len(scaffold_result):,} rows to "
                    f"data/raw/catch_prob_plays_{scaffold_year}.parquet."
                )
            else:
                st.warning("Fetch failed or returned no data — check logs / network access.")
...
df_all = _cached_dashboard(_df_hash(cp_raw), _df_hash(ss_raw), cp_raw, ss_raw)
df     = filter_df(df_all,
...
with tab_table:
    ...
    for rank, (_, row) in enumerate(df_sorted.iterrows(), 1):
        ...
        disp_rows.append({
            ...
            "Park Adj. OAA": fmt_oaa(row.get("park_adj_oaa")),
    ...
    display_df = pd.DataFrame(disp_rows)
    st.dataframe(display_df, use_container_width=True,
                 height=520, hide_index=True)

    csv_bytes = display_df.to_csv(index=False).encode("utf-8")
    seasons_tag = "-".join(str(s) for s in sorted(selected_seasons))
    bracket_tag = active_bucket.label.split(" ")[0]  # e.g. "5-Star"
    st.download_button(
        "⬇️ Download table as CSV",
        data=csv_bytes,
        file_name=f"mlb_defense_{bracket_tag}_{seasons_tag}.csv",
        mime="text/csv",
        use_container_width=False,
        help="Downloads exactly the rows and columns shown above, in current sort order.",
    )

    park_note = (
        "Real park factors loaded from data/raw/park_factors.csv."
        if PARK_FACTORS_PATH.exists()
        else "No data/raw/park_factors.csv found, so **Park Adj. OAA** currently equals **Total OAA** "
             "(neutral factor 1.00 for every team)."
    )
    st.caption(
        "**Avg CP% (est.)** — directional estimate, not a per-play mean. "
        f"Shown as “—” when a bracket has fewer than {MIN_BUCKET_SAMPLE} opportunities "
        "(too few plays for the estimate to be meaningful). "
...
with tab_buckets:
    ...
    margin=dict(t=200, l=160, r=40, b=50),   # duży top margin
    title=dict(
        ...
        xanchor="center",
            ...
            if heat_r:
                ...
                fig_h = go.Figure(go.Heatmap(
                    ...
                    textfont={"size":11},
                    hovertemplate="Player:%{y}<br>Bracket:%{x}<br>Avg CP:%{text}<extra></extra>",
    ...
    title=dict(
        ...
        xanchor="center",
                ...
                st.caption(
                    "Lower Avg CP% within a bracket means harder plays. "
                    "A 5-Star player at 5% avg CP faced near-impossible plays; "
                    "at 22% the plays were difficult but more achievable. "
                    f"“—” = fewer than {MIN_BUCKET_SAMPLE} opportunities in that bracket "
